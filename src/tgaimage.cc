...
bool TGAImage::ReadFile(const std::string filename) {
  ...
  if (width <= 0 || height <= 0 ||
      (bytespp != Color::kGrayScale && bytespp != Color::kRGB &&
       bytespp != Color::kRGBA)) {
    in.close();
...
bool TGAImage::WriteFile(const std::string filename, const bool vflip,
  ...
  header.datatypecode =
      (bytespp == Color::kGrayScale ? (rle ? 11 : 3) : (rle ? 10 : 2));
...
bool LoadTgaImage(std::string const &path, Data2D *data2d) {
  ...
  for (int x = 0; x < img.width; x++) {
    for (int y = 0; y < img.height; y++) {
      ...
      if (img.bytespp >= 3) {
        // bgra -> rgba
        std::swap(img.data.at(idx), img.data.at(idx + 2));
      }
    }
  }
  *data2d = Data2D(img.width, img.height, img.bytespp, std::move(img.data));
  return true;
}
